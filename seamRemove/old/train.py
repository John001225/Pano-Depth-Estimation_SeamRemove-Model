...
root = './dataset'
save_path = 'checkpoints_stitched2'
os.makedirs(save_path, exist_ok=True)

writer = SummaryWriter(log_dir='runs/seam_removal_stitched_2')
...
for epoch in range(epochs):
    ...
    train_pbar = tqdm(train_loader, desc=f"Epoch [{epoch+1}/{epochs}] Training", dynamic_ncols=True)
    ...
    model.eval()
    val_loss = 0.0
    val_pbar = tqdm(val_loader, desc=f"Epoch [{epoch+1}/{epochs}] Validation", dynamic_ncols=True)
    with torch.no_grad():
